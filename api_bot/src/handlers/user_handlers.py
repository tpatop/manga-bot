...
@router.callback_query(F.text == '/manga_add')
...
@router.message(F.text.startswith(f'{URL_MANGA[0]}/'))
@router.message(F.text.startswith(f'{URL_MANGA[1]}/'))
@router.message(F.text.startswith(f'{URL_MANGA[2]}/'))
...
@router.callback_query(F.data == '/manga_target')
...
@router.callback_query(F.data == '/manga_delete')
...
@router.callback_query(F.text.startswith('del*'))
...
# @router.callback_query(F.text.startswith('rev*'))
...
#         callback.from_user.id, hash_name)
...
# @router.callback_query(F.text == '/add_manga_in_target')
...
# @router.callback_query(F.text == '/del_manga_in_target')
...
#     hash_name = await hash_full_text(name)
#     await delete_manga_from_target(
#         hash_name, callback.message.chat.id)
#     await bot.answer_callback_query(callback.id,
...
async def show_settings_for_user(callback: CallbackQuery, **kwargs):
    markup = await manga_settings_kb(callback.from_user.id)
    await callback.message.edit_text(
...
@router.callback_query(F.text == '/all_target_true')
@router.callback_query(F.text == '/all_target_false')
...
@router.callback_query(F.data == '/status_live_true')
@router.callback_query(F.data == '/status_live_false')
...
# @router.callback_query(F.data == '/show_update')
...
# @router.callback_query(F.data.startswith('/showup*'))
...
# async def process_semd_to_all_live_users(message: Message, bot: Bot):
#     await bot_send_to_all_user_live(bot, message.text)


# # Для Подсчета живых пользователей
# @router.message(F.text == '/stat')
# async def process_send_statistic(message: Message, bot: Bot):
#     await bot_send_statistic(bot, message.chat.id)
